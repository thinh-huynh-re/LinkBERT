...
def organize_mrqa():
    ...
    for data_file, output_dir in tqdm(zip(mrqa_raw_files, mrqa_dataset_names)):
        os.system(
            f"cp -rp raw_data/mrqa/train/{data_file} {root}/{output_dir}/train.jsonl"
        )
        os.system(
            f"cp -rp raw_data/mrqa/dev/{data_file} {root}/{output_dir}/dev_mrqa.jsonl"
        )
